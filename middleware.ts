 ...
 *
 * 팀이 공유하는 것은 비밀번호 하나다. 아이디는 비밀이 아니라서 따로
 * 요구해봐야 외울 것만 늘고 막아주는 것은 없다.
 *
 * 프로토콜이 아이디:비번 쌍을 요구해서 브라우저 창의 아이디 칸을 없앨 수는
 * 없다. 그래서 **비워야만 통과**시킨다. 아무 값이나 받아주면 "무엇을 넣어도
 * 되는지" 가 화면에 드러나지 않아, 사람마다 다르게 넣고 다르게 기억한다.
 * 규칙이 하나면 공지도 한 줄이다 — "아이디는 비우세요".
 *
...
export function credentialsAccepted(
	...
	// 지정한 아이디가 없으면 빈 값이어야 한다. 아무 값이나 받지 않는다.
	const okUser = constantTimeEquals(
