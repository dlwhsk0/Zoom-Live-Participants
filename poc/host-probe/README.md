...
> **이 도구는 역할을 다했고, 결론은 "봇은 답이 아니다" 였다.**
>
> 현재 호스트는 `meeting.participant_role_changed` 웹훅으로 판정한다.
> 상세는 [`docs/host-detection.md`](../../docs/host-detection.md).
>
> 그래도 지우지 않고 남긴다 — **봇으로는 소회의실이 보이지 않는다**는 결론의
> 근거가 이 안에 있고, 나중에 같은 길을 다시 떠올릴 때 여기부터 보면 된다.
