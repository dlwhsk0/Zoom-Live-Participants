...
주소는 타일에 담기엔 길다. 걷어내고 ▶ 표시로 대신한다. `<a>` 는 `<button>`
안에 넣을 수 없으므로 글(버튼)과 링크를 나란히 둔다.

새 탭은 `rel="noopener noreferrer nofollow"` 로 연다.

**글자 수 제한과의 관계:** 30자 안에 주소가 들어가야 한다.
`youtu.be/dQw4w9WgXcQ` 는 20자라 글 쓸 자리가 10자 남는다.
`https://` 를 붙이면 28자라 거의 남지 않는다.
