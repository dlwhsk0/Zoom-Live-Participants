...
타일 아래에 짧은 상태 메시지를 둔다. **50자까지.**

그리드로 바꾸면서 한 칸이 90px 남짓으로 좁아졌다. 60자를 그대로 두니
타일을 넘어 옆 칸을 침범해서 줄였다. 화면에서는 이렇게 받는다.
...
줄여도 넘치면 줄을 바꾸고, 세 줄을 넘기면 자른다. 전문은 호버하면
말풍선으로 뜬다. 말풍선을 버튼 안에 두면 버튼의 `overflow: hidden` 에
같이 잘리므로, 잘리지 않는 껍데기(`.status-wrap`)를 하나 두고 거기서 그린다.

제한보다 긴 메시지가 이미 저장돼 있으면 그대로 남는다. 다시 저장할 때만 걸린다.

##### 타일을 누르면 프로필 카드가 열린다

타일은 90px 이라 이름과 시간밖에 못 담는다. 상태 메시지는 세 줄에서 잘리고
유튜브 주소는 아예 못 보여준다. 그래서 **타일 전체가 프로필 카드를 여는
버튼**이고, 자세한 것은 카드에서 본다.
...
**수정은 카드에서 들어간다.** 예전에는 타일의 상태 메시지가 곧 수정 버튼이었는데,
타일 전체가 눌리게 되면서 서로 먹는다. 90px 안에 누를 것을 여럿 두면 안 된다.

카드는 `participantUuid` 만 붙들고 목록에서 최신 데이터를 매번 다시 찾는다.
5초마다 갱신되므로 카드를 열어둔 채로 시간이 흐른다.

###### 영상 제목은 oEmbed 로 가져온다

`https://www.youtube.com/oembed?url=...&format=json` 은 **키가 필요 없고
CORS 도 열려 있다.** 브라우저가 직접 부르므로 우리 서버를 거치지 않는다 —
서버가 남이 준 주소로 요청하는 구조를 만들지 않아도 된다(SSRF 여지 없음).

부르기 전에 반드시 `parseYoutube` 를 통과한 주소여야 한다. 허용 호스트가
아니면 거기서 걸린다.

제목은 **남이 지은 글**이다. 반드시 텍스트로만 그린다. React 가 escape 하므로
`dangerouslySetInnerHTML` 을 쓰지 않는 한 안전하다.

없는 영상이거나 비공개면 400 이 온다. 그때는 "유튜브에서 열기" 로 물러선다 —
제목을 못 가져와도 링크는 열 수 있어야 한다.

같은 영상은 react-query 가 붙들고 있어 카드를 다시 열어도 다시 받지 않는다.

##### 유튜브 링크만 열어준다

상태 메시지에 유튜브 주소를 적으면 ▶ 표시가 붙고 눌러서 열 수 있다.

**아무 주소나 링크로 만들지 않는다.** 상태 메시지는 인증 없이 누구나 쓸 수
있으므로, 링크를 열어주면 남의 타일에 피싱 주소를 걸 수 있다.
호스트 허용 목록을 두고 그 밖은 글자로만 남긴다(지우지는 않는다).

판정은 문자열 패턴이 아니라 `new URL()` 파싱으로 한다. 프로토콜과 호스트를
직접 봐야 `javascript:` 같은 스킴과 `youtube.com.evil.kr` 같은 흉내를 막는다.

```
youtu.be/abc                 → 링크 (스킴이 없으면 https 를 붙인다)
https://www.youtube.com/...  → 링크
https://youtube.com.evil.kr  → 거절
javascript:alert(1)          → 거절
```

주소는 타일에 담기엔 길다. **타일에서는 걷어내고 ▶ 를 앞에 붙인다** —
재생목록 한 줄처럼 읽히게 한 것이고, 누를 수 없는 표시다.
진짜 링크는 프로필 카드에 있다. 타일 전체가 버튼이라 그 안에 `<a>` 를
둘 수 없기도 하다.

▶ 는 글 흐름 **안에** 둔다. 옆에 떼어 두면 글이 줄바꿈될 때 따로 놀고,
빈 칸이 폭을 다 먹어 오른쪽 끝으로 밀린다. 실제로 그렇게 만들었다가 고쳤다.

적은 글이 없으면 영상 제목이 그 자리를 채운다. 링크만 덩그러니 있는 것보다
무엇을 걸었는지 보이는 편이 낫다.

새 탭은 `rel="noopener noreferrer nofollow"` 로 연다.

**글자 수는 50자다.** 주소도 이 안에 들어간다.
`https://youtu.be/dQw4w9WgXcQ` 가 28자이므로 글 쓸 자리가 22자 남는다.
처음엔 30자였는데 주소를 넣으면 글을 못 써서 늘렸다.

##### 넘친 글자는 붉게 깔아 보여준다

트위터처럼 제한을 넘겨도 계속 칠 수 있게 두고, 넘친 부분만 붉게 깐다.
글자 수는 `36/30` 처럼 보여주고 넘기면 붉어지며 저장이 막힌다.

입력칸 안의 텍스트에는 글자마다 스타일을 줄 수 없다. 그래서 **두 겹**으로 만든다.

```
거울(div)     같은 글을 그린다. 넘친 부분만 <mark> 로 감싼다
입력칸        위에 겹친다. 글자는 투명, 커서만 보인다(caret-color)
```

두 겹의 글자 배치가 어긋나면 안 되므로 폰트·여백·줄바꿈 규칙을 공유 규칙에
몰아 두었다. 하나만 고치면 틀어진다.

한 줄짜리 `input` 이 아니라 `textarea` 인 이유는 가로 스크롤 때문이다.
`input` 은 길어지면 안에서 스크롤되는데 거울은 따라가지 못해 어긋난다.
줄바꿈으로 받으면 두 겹이 항상 붙어 있다.
...
- 소회의실 위치 표시 (판정에는 쓰지 않는다. **"알 수 없다"는 2026-09-12 에 뒤집혔다** —
  `breakout_room_uuid` 로 방을 구분할 수 있다. 상세는 `docs/webhook-data-reference.md`)
- 입퇴장 이력 전체(누가 언제 몇 번 드나들었는지), 통계, 검색, 정렬 옵션
- 다크모드, 애니메이션

**완료 기준:** 폰 브라우저에서 열어두면 사람이 들어오고 나갈 때마다 목록이 바뀐다.

### 팝업은 portal 로 body 에 그린다

`.card--offline` 에 `opacity: 0.72` 가 있다. **1 보다 작은 opacity 는
stacking context 를 만든다.** 그 안에서는 `z-index` 가 아무리 커도 바깥
형제들보다 위로 올라가지 못한다.

상태 메시지 팝업이 타일 안에 그려지고 있어서 `z-index: 50` 이 그 카드
안에 갇혔고, DOM 상 뒤에 오는 타일들이 위를 덮었다. 모바일은 격자가
촘촘해 특히 심해서 수정 자체가 안 됐다.

`createPortal` 로 body 바로 아래에 그린다. stacking context 를 통째로
빠져나가므로 앞으로 어떤 CSS 가 추가돼도 안전하다.

서버 렌더에는 `document` 가 없다. `renderToString` 을 쓰는 테스트와
미리보기에서는 제자리에 그린다.
...
한 사람이 노트북과 폰으로 동시에 접속하면 구간이 겹친다. 그냥 더하면
두 배가 된다.
...
그래서 서버는 구간 목록을 만들어(`findIntervals`) 합집합 길이를 낸다
(`unionSeconds`). 진행 중인 구간은 `now` 까지로 본다.

**화면은 이 값을 그대로 쓴다.** 예전에는 서버가 닫힌 구간만 주고 화면이
진행 중인 구간을 더했는데, 겹침을 눌러야 하므로 그 방식으로는 맞출 수 없다.
표시 단위가 분이라 폴링 간격(5초)만큼의 지연은 드러나지 않는다.

증상과 원인은 `plan.md` 의 "한 사람이 여러 기기로 동시에 접속" 참고.
...
`/logs?key=...` 는 이제 탭이 셋인 어드민이다. **사람 · 로그 · 기록.**

##### 왜 필요한가

병합은 이름으로만 판단한다. 그래서 두 가지가 틀어질 수 있다.

- **한 사람이 여러 명으로 보인다** — 중간에 Zoom 표시 이름을 바꾼 경우
- **여러 사람이 한 명으로 보인다** — 동명이인

둘 다 이름을 고쳐서 해결한다. **합치기와 떼어내기가 같은 동작이다.**
이름을 같게 하면 합쳐지고 다르게 하면 떨어진다. 규칙이 하나뿐이라
따로 만들 것이 없었다.

##### 왜 이름 별칭 테이블이 아닌가

처음에 `참가자02 → 현승곤` 같은 매핑 테이블을 생각했다. 하지만 그건
이름→이름 매핑이라 **여러 사람이 서로 이름을 바꾸면 매핑이 뒤엉킨다.**

행을 직접 고치면 신원이 특정 `participant_uuid` 에 붙으므로 그런 일이 없다.
같은 이름이 나중에 다른 사람에게 쓰여도 과거 행에는 영향이 없다.

##### 별칭 — 고정 닉네임 (별칭 탭)

일부 사람은 회의마다 정해진 닉네임을 쓴다. `Chloe = 이도경` 같은 경우다.
행 수정은 그 세션에만 적용되므로 다음 회의에서 또 갈린다.
별칭은 **모든 세션에 계속 적용**된다.

```
행 수정   그 세션의 특정 행만       일회성 교정
별칭      모든 세션에 계속 적용     고정 닉네임
```

조회 시점에 이름을 갈아 끼운다. 병합이 이름으로 판단하므로 합치기 전에
대표 이름으로 바꿔 두면 그대로 한 사람이 된다. 원본 행은 그대로다.

**사슬(A→B→C)은 막는다.** 한 단계만 치환해야 결과를 예측할 수 있다.
양쪽을 다 본다 — 새 대표가 이미 별칭이어도, 새 별칭이 이미 다른 별칭의
대표여도 거절한다.

###### IP 로 자동 연결하지 않는 이유

운영 데이터에서 같은 공인 IP 를 쓰는 이름 조합을 전수로 찾았더니 한 쌍 나왔다.

```
175.114.218.56   Kevin, 김동현

  Kevin   17:19:55 ~ 17:20:10
  김동현   17:19:58              ← 3초 차이로 둘 다 접속 중
```

**같은 공유기 뒤의 서로 다른 두 사람이다.** IP 로 자동 연결하면 이 둘이
한 명으로 뭉친다. 별칭은 어드민이 손으로 선언한다.

###### 접속 중 이름을 바꾸는 경우는 이미 처리된다

별칭이 필요 없는 경우도 있다. 접속을 유지한 채 줌 이름만 바꾸면
`participant_uuid` 가 그대로라 `participants` 행이 최신 이름으로 덮어써진다.

```
이도경   uuid 41DE7E16   11:26:38 ~ 11:55:26
Chloe   uuid 41DE7E16   11:55:26          ← 같은 uuid
```

별칭이 필요한 것은 **접속을 끊고 다른 이름으로 다시 들어올 때**다.
그때는 uuid 가 새로 발급돼 두 명으로 갈린다.

##### 무엇을 고치고 무엇을 남기는가

```
participants.display_name    고친다
participant_events           손대지 않는다   Zoom 이 보낸 이름 그대로
webhook_events               손대지 않는다   원본 payload 그대로
admin_actions                편집을 기록한다  이전 값 포함
```

원본을 남기므로 로그 탭에는 여전히 **Zoom 이 실제로 보낸 이름**이 보인다.
어드민이 고친 것은 화면에 보이는 집계뿐이다.

##### 되돌리기

`admin_actions.detail` 에 각 행의 이전 이름을 담는다. 기록 탭의
"되돌리기" 가 그 값을 되돌려 놓는다. 되돌린 것도 기록에 남는다.

##### 엔드포인트

로그와 같은 `LOGS_TOKEN` 으로 막는다.

| | |
|---|---|
| `GET /api/admin/identities` | 합치기 전의 원본 행 |
| `POST /api/admin/rename` | 고른 행의 이름을 하나로 |
| `GET /api/admin/actions` | 고친 기록 |
| `POST /api/admin/undo` | 되돌리기 |
| `GET /api/admin/aliases` | 별칭 목록 |
| `POST /api/admin/aliases` | 별칭 추가·수정 |
| `DELETE /api/admin/aliases?alias=` | 별칭 삭제 |

##### 로그에서 동시 접속을 알아보기

한 사람이 노트북과 폰으로 같이 들어오면 로그가 헷갈린다. 같은 이름이 두 번
들어오고 한 번만 나간 것처럼 보인다. 그래서 로그 탭에 태그를 붙인다.

| 태그 | 뜻 |
|---|---|
| `동시 접속` | 이 입장 시점에 같은 사람의 다른 접속이 이미 살아 있었다 |
| `다른 기기 접속 중` | 이 퇴장 뒤에도 같은 사람의 다른 접속이 남아 있다 |

실제로 이렇게 읽힌다.

```
23:15:37  입장  조하나  [동시 접속]                    ip 118.235.13.75
23:16:22  퇴장  조하나  [방 이동][다른 기기 접속 중]
23:16:32  퇴장  조하나  [다른 기기 접속 중]
23:14:03  퇴장  조하나  [방 이동]                      ip 175.197.56.194
```

IP 대역이 달라 유선과 모바일임이 바로 보인다. 마지막 퇴장에도 태그가
붙어 있으므로 "나갔는데 왜 접속 중이지" 가 풀린다.

사람은 **이름으로** 묶는다. 병합 규칙과 같은 기준이어야 로그와 화면이
어긋나지 않는다. 별칭도 같이 적용한다. 같은 `participant_uuid` 는
제외한다 — 소회의실 이동은 한 uuid 안에서 일어나므로 동시 접속이 아니다.

재접속 인수인계(새 연결의 `joined` 가 옛 연결의 `left` 보다 먼저 오는
구간)도 이 태그에 걸린다. 기록상 실제로 겹치는 것이 맞다.

##### 탭은 주소에 남는다

`?tab=logs` 처럼 열린 탭이 주소에 남는다. 새로고침해도 보던 탭이
유지되고 링크로 공유할 수 있다.
