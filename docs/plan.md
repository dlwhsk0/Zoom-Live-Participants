...
노트북과 폰으로 같이 들어오는 경우다. 이름이 같으므로 한 사람으로 합쳐지는데,
합치는 방식이 두 가지 면에서 틀려 있었다. 둘 다 겹침 안전장치를 뺀 뒤에야
드러났다 — 그전에는 겹치는 두 행이 아예 합쳐지지 않아 다른 형태로 틀렸다.

#### 증상 1 — 하나가 나가면 오프라인이 된다

```
노트북   14:00 입장 ~ (계속 접속 중)
폰       14:10 입장 ~ 14:20 퇴장
                        ↑ 여기서 화면이 오프라인으로 뒤집힘
```

`absorb` 가 합쳐진 사람의 `isPresent` 를 **뒤 행의 값으로 덮어쓰고** 있었다.
행은 최초 입장 순으로 정렬되므로 나중에 들어온 폰이 뒤 행이고,
폰이 먼저 나가면 그 값이 그대로 결과가 됐다.
...
**한 사람은 접속이 하나라도 살아 있으면 회의에 있는 것이다.**
...
폰       14:10 ~ 14:20   10분
                         ────
합계                     40분   ← 틀림. 사람은 한 명이니 30분이다
```

구간을 그냥 더하고 있었다. 겹치는 부분이 두 번 셈해진다.

**구간을 합집합으로 누른다**(`unionSeconds`). 시작 시각 순으로 정렬한 뒤,
앞 구간과 겹치거나 맞닿으면 끝만 늘리고 떨어져 있으면 앞 구간을 확정한다.

#### 곁들여 옮긴 것 — 누적 시간을 어디서 계산하는가

예전에는 서버가 **닫힌 구간만** 주고 화면이 진행 중인 구간을 더했다.
겹침을 눌러야 하니 이 방식으로는 맞출 수 없다. 화면은 구간 목록을
받지 않기 때문이다.

서버가 `now` 까지 끝까지 계산해서 준다. 표시 단위가 분이라
폴링 간격(5초)만큼의 지연은 화면에 드러나지 않는다.

#### 남는 한계

같은 이름 두 사람이 실제로 같이 있으면 여전히 한 명으로 합쳐지고,
누적 시간도 합집합으로 눌린다. 동명이인은 어드민에서 한쪽에 다른 이름을
주어 떼어낸다. 상세는 `implementation-plan.md` 의 어드민 화면 참고.
