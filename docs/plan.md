...
세션 안에서 재접속한 사람을 한 명으로 합치려면 `이름 + public_ip` 를 쓴다.

| 후보 | 판정 |
|---|---|
| `public_ip` | ✅ 재접속 7건 전부 유지됨. 와이파이 끊김 케이스 포함 |
| `user_id` | ❌ 방을 옮길 때마다 바뀐다. 한 세션에서 9개 관측 |
| `email` | ❌ `joined`/`left` 에서 0%. 소회의실 이벤트에서만, 그것도 일부 |
| `participant_user_id`, `id` | ❌ 157건 중 1건만 채워짐 |

`public_ip` 단독은 유일하지 않다(같은 NAT 뒤에 여러 명). 이름과 함께 써야 한다.
그리고 **활동 구간이 겹치면 합치지 않는다** — 한 사람이 동시에 두 곳에 있을 수 없으므로,
겹치는 두 행은 같은 이름 같은 IP 라도 다른 사람이다. 같은 교실의 동명이인이 이 경우다.
