...
주의: `participant_uuid`는 **회의 세션 단위**로 발급된다.
같은 사람이 다른 날 다시 들어오면 다른 값을 받는다.
따라서 "지금 접속 중인가"에는 쓸 수 있지만, **사람 단위의 영속 식별자가 아니다.**
날짜를 가로지르는 사용자 식별이 필요해지면 그때 별도 설계가 필요하다 (이번 범위 밖).
