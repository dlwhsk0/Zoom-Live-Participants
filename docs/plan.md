...
IP 를 빼도 동명이인이 뭉치지는 않는다. **시간 겹침 안전장치**가 남아 있어서,
같은 이름 두 사람이 실제로 같이 있으면 합쳐지지 않는다 —
한 사람이 동시에 두 곳에 있을 수 없기 때문이다.

운영 9세션 전수 확인: 같은 이름이 2행 이상인 경우 8건, 그중 시간이 겹쳐
동명이인으로 의심되는 경우 **0건**.

남는 위험은 동명이인 두 사람이 서로 **한 번도 같이 있지 않은** 경우다.
그때는 어드민에서 손으로 떼어낸다.
