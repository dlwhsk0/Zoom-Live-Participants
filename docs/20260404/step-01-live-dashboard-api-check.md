...
새 Zoom 앱 자격증명과 새 회의 ID `10000000001` 기준으로 Dashboard live meeting / live participants API를 다시 검증한다.
...
- `.env` 에 `ZOOM_MEETING_ID=10000000001` 이 설정되어 있다.
...
```text
status: 400
url: https://api.zoom.us/v2/metrics/meetings/10000000001?type=live
...
```text
status: 400
url: https://api.zoom.us/v2/metrics/meetings/10000000001/participants?type=live&page_size=300
message: This API is only available for ZMP and Business or higher accounts that have enabled the Dashboard feature.
```
...
- 새 Zoom 앱 자격증명 + 새 회의 ID `10000000001` 기준으로도 `live meeting`, `live participants` API는 실패한다.
