...
## 세션 정보 — `payload.object`

참가자 정보 옆에 세션 자체의 정보가 함께 실려 온다.

| 필드 | 채워짐 | 성격 |
|---|---:|---|
| `uuid` | 157/157 | 세션 식별자. 회의를 새로 열 때마다 바뀐다 |
| `id` | 157/157 | 회의방 번호. 세션이 바뀌어도 고정 |
| `start_time` | 157/157 | **세션 시작 시각. 세션 상수다** |
| `host_id` | 157/157 | 회의방을 소유한 계정. 방마다 고정 |
| `host_email` | 일부 | 소유 계정 메일 |
| `topic` | 157/157 | 회의 제목 |
| `timezone` | 157/157 | 호스트 계정의 시간대 |

### `start_time` 은 세션 상수다

fixture 4개 세션 전수 확인. 세션 안에서는 항상 같고, 세션끼리는 전부 다르다.

| 세션 | `start_time` | 이벤트 |
|---|---|---:|
| `TESTUUID0001==` | `2026-04-03T10:56:58Z` | 33 |
| `TESTUUID0002==` | `2026-04-11T06:33:13Z` | 15 |
| `TESTUUID0003==` | `2026-04-11T10:20:19Z` | 4 |
| `TESTUUID0004==` | `2026-04-11T11:56:09Z` | 110 |

### `host_id` 로는 문 연 사람을 알 수 없다

`host_id` 는 **회의방을 소유한 계정**이다. 오늘 누가 문을 열었는지가 아니라
방을 만든 계정이라 값이 늘 같고, 참가자의 `user_id` 와도 이어지지 않는다.

```
host_id      YjEAL8l3S5SwiUZT-8Dgrg
...
```
start_time            05:59:31
가장 이른 joined       05:59:31   현승곤
role_changed          05:59:36   현승곤
```

**`meeting.started` 웹훅을 못 받아도 세션 시작 시각을 알 수 있다는 뜻이다.**
서버를 늦게 켜서 앞부분을 통째로 놓쳤어도, 그 뒤에 도착한 아무 웹훅 하나면
시작 시각이 복구된다. 상세는 `time-and-gaps.md` 개선안 7 참고.
...
현재 구현은 원본만 저장하고 접속 판정에는 쓰지 않는다.
**이 이벤트 없이도 판정이 정확하다는 것이 실제 회의에서 확인됐다.**

### 방을 구분할 수 있다 — `breakout_room_uuid`

2026-09-12 실측(2818건, 운영 DB):

| 필드 | 채움률 | 성격 |
|---|---:|---|
| `breakout_room_uuid` | **100%** | **방 식별자.** object 바로 아래에 있다(참가자 안이 아니다) |
| `parent_user_id` | 100% | 일반 이벤트의 `user_id` 와 이어진다 — 한 세션에서 40/40 일치 |

`implementation-plan.md` 에 "소회의실 위치는 애초에 알 수 없다"고 접어 둔
항목은 **틀렸다.** 누가 어느 방에 있는지 알 수 있다. 세션당 방은 1~10개였다.

### 방 이름은 알 수 없다

구분은 되지만 **이름은 붙일 수 없다.** 이벤트의 `topic` 은 회의 제목이라 모든
방이 같은 값을 쓴다(방 159개 전부 "테커 24시간 줌 미팅룸").

이름 자체는 회의 설정에 있다 — `GET /meetings/{id}` 의
`settings.breakout_room.rooms[]` 가 사전 배정된 방 13개의 이름을 준다
(`Samsung`, `OpenAI`, `Mute 1 (Cam On)` 등). 그런데 그 목록과 웹훅의
`breakout_room_uuid` 를 잇는 고리가 없다.

`breakout_room_uuid` 는 **세션마다 새로 만들어진다**(세션을 넘어 재사용된 방
0건). 그래서 사람이 한 번 이어 준다 해도 다음 세션에는 쓸 수 없다.

화면에 쓸 수 있는 것은 "같은 방에 있는 사람들" 까지다.

### 다만 그대로 세면 과다 집계된다

한 세션에서 입장 32건 · 퇴장 21건으로 **11건이 짝을 잃는다.** 소회의실에서
곧바로 회의를 나가면 `left_breakout_room` 이 오지 않기 때문이다. 그 상태로
방별 인원을 세면 메인 접속자가 11명인데 어떤 방에 21명이 있는 것으로 나온다.

그래서 방 위치를 쓰려면 **메인 세션의 접속 여부로 한 번 걸러야 한다.**
`parent_user_id` → 일반 이벤트 `user_id` 로 이어서, 지금 접속 중인 사람만 남긴다.

### 그 밖에 주의할 점

- 이 이벤트의 `user_id` 는 형식이 다르다. 예: `"167782402026-08-30 09:00:49:000"`
  (숫자 뒤에 타임스탬프가 붙는다). 일반 `joined`/`left` 의 `user_id` 와 같은
  네임스페이스가 아니다. 이을 때는 반드시 `parent_user_id` 를 쓴다.
- `email` 은 **방이 아니라 사람의 것**이다. 로그인한 참가자에게만 드물게 채워져서
  방마다 한둘씩 보이는 탓에 방 단위처럼 읽혔을 뿐이다(2026-09-12 재확인: 같은
  이메일이 여러 방에 나타난다 — 그 사람이 방을 옮긴 것이다).
  방을 구분할 목적이라면 `breakout_room_uuid` 를 쓴다.
