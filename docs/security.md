...
**아이디는 검사하지 않는다.** 프로토콜이 아이디:비번 쌍을 요구해서 브라우저
창의 아이디 칸을 없앨 수는 없지만, 아무 값이나 넣어도(비워도) 통과한다.
팀이 공유하는 것은 비밀번호 하나면 된다 — 아이디는 비밀이 아니라서 따로
요구해봐야 외울 것만 늘고 막아주는 것은 없다. `BASIC_AUTH_USER` 를 굳이
지정하면 그때는 아이디도 함께 본다.
