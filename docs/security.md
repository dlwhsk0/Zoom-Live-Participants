...
```
바깥문  사이트 전체        공용 비밀번호 (HTTP Basic)   ← 만들었지만 켜지 않음
...
### 바깥문 — HTTP Basic (현재 꺼짐)

**2026-09-10 기준 켜지 않았다.** 한 번 켰다가 껐다 — 팀 운영 쪽 판단으로
"참가자에게 비밀번호를 요구하는 것은 과하다" 였다. 코드는 그대로 두었고,
`BASIC_AUTH_PASSWORD` 를 넣고 재배포하면 다시 켜진다.

이 문이 없으면 아래 API 공유 토큰의 성격이 달라진다(그 절 참고).
...
`BASIC_AUTH_PASSWORD` 를 넣는 순간부터 잠긴다.
`robots.txt` 는 통과시킨다. 크롤러가 "들어오지 마라" 는 읽어야 한다.

**아이디 칸은 비워야 한다.** 프로토콜이 아이디:비번 쌍을 요구해서 브라우저
창의 아이디 칸을 없앨 수는 없다. 그래서 빈 값만 통과시킨다 — 아무 값이나
받아주면 "무엇을 넣어도 되는지" 가 화면에 드러나지 않아 사람마다 다르게
넣고 다르게 기억한다. 규칙이 하나면 공지도 한 줄이다.

> 팀 공지: **아이디는 비우고 비밀번호만 넣으세요.**

비밀번호 관리자가 아이디를 자동으로 채우면 비밀번호가 맞아도 막힌다.
`BASIC_AUTH_USER` 를 지정하면 그때는 그 값과 같아야 한다.

Vercel 엣지에는 **시도 제한이 없다.** 어드민 로그인(10분에 10번)과 달리
무제한으로 시도할 수 있으므로, 비밀번호는 추측되지 않는 값이어야 한다.
...
토큰은 화면 번들에 박혀 나간다. 원래 설계는 **감추는 값이 아니라 번들을 받을
수 있는 사람에게만 주는 값** 이었다 — 사이트가 공용 비밀번호 뒤에 있으면
토큰을 얻으려면 먼저 그 문을 통과해야 한다.
...
그래도 남겨 둔 이유는 비용이 0 이고, 나중에 문을 다시 세우거나 다른 방식
(예: 로그인한 뒤에만 토큰을 내려주기)으로 갈 때 기반이 되기 때문이다.

**"API 가 막혀 있다" 고 믿으면 안 된다.** 참가자 실명은 사실상 공개 상태다.
...
| `VITE_ACCESS_TOKEN` | Production | 조회 API 토큰. 번들에 박힌다 — 공개 전제 |
| `VITE_ADMIN_PATH` | Production | 어드민 화면 주소 |
| `PORT` | Production | 정적 사이트에 의미 없음. 지워도 그만 |

`BASIC_AUTH_PASSWORD` 는 지금 넣지 않았다. 넣고 재배포하면 사이트가 잠긴다.
`BASIC_AUTH_USER` 도 비워 둔다 — 그러면 브라우저 창의 아이디 칸도 비워야 통과한다.
